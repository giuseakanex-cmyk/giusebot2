...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    if (command === 'taglia') {
        ...
        if (!target) target = global.db.data.users[who] = { euro: 0, taglia: 0 }
        if (!target.taglia) target.taglia = 0

        user.euro -= importo
        target.taglia += importo // Si somma alle taglie precedenti!

        let msg = `
╭━━✧🏴‍☠️ WANTED 🏴‍☠️✧━━╮
🩸 È STATA MESSA UNA TAGLIA!
👤 Bersaglio: @${who.split('@')[0]}
💰 Ricompensa: *${formatNumber(target.taglia)} €*
...
╰━━━━━━✧✦━━━━━━━╯`.trim()
    ...
    if (command === 'spara') {
        ...
        if (!target || !target.taglia || target.taglia === 0) return m.reply(`🤷‍♂️ Non c'è nessuna taglia su questa persona. Non sprecare proiettili!`)
        ...
        if (hit) {
            let vincita = target.taglia
            user.euro += vincita
            target.taglia = 0 // Taglia azzerata
            target.nascosto = 0 
...
Banca attuale: *${formatNumber(user.euro)} €*`.trim()
    ...
    if (command === 'nasconditi') {
        if (user.taglia === 0) return m.reply(`Nessuno ti sta cercando, perché ti nascondi?`)
...
handler.command = ['taglia', 'spara', 'nasconditi']
handler.group = true
